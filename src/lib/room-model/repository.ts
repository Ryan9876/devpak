...
async function hydrateRoom(supabase: SupabaseClient, room: any): Promise<RoomModel> {
  ...
  return {
    ...
    units: room.units,
    boundary: {
      widthUm: Number(room.width_um),
      depthUm: Number(room.depth_um),
      ceilingHeightUm: room.ceiling_height_um == null ? null : Number(room.ceiling_height_um),
    },
...
export async function listProjects(supabase: SupabaseClient, ownerId: string): Promise<ProjectSummary[]> {
  const { data, error } = await supabase
    .from('projects')
    .select('id,name,default_units,updated_at')
    .eq('owner_id', ownerId)
    .is('deleted_at', null)
  ...
  return (data ?? []).map((project: any) => ({
    id: project.id,
    name: project.name,
    defaultUnits: project.default_units,
    updatedAt: project.updated_at,
  }));
}

export async function loadProjectRoom(
  ...
  const { data: room, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw error;
  return room ? hydrateRoom(supabase, room) : null;
}
...
export async function createProjectWithStarterRoom(
  supabase: SupabaseClient,
  ownerId: string,
  requestedName: string,
) {
  const name = requestedName.trim().slice(0, 120) || 'Untitled room';
  const { data: project, error: pe } = await supabase
    .from('projects')
    .insert({ owner_id: ownerId, name, default_units: 'imperial' })
    .select()
    .single();
  if (pe) throw pe;
  ...
  try {
    const { data: room, error: re } = await supabase
      ...
      .insert({
        ...
        depth_um: 3_048_000,
        units: 'imperial',
        assumptions: ['Initial dimensions are examples until you replace or verify them.'],
      })
      .select()
      .single();
    if (re) throw re;
    ...
    const { error: oe } = await supabase.from('room_objects').insert(objects);
    if (oe) throw oe;
    const { error: op } = await supabase.from('room_openings').insert({
      room_id: room.id,
      owner_id: ownerId,
      wall: 'south',
      offset_um: 1_000_000,
    ...
    await supabase.from('projects').delete().eq('id', project.id).eq('owner_id', ownerId);
    throw error;
  }
}

export async function createStarterRoom(supabase: SupabaseClient, ownerId: string) {
  const created = await createProjectWithStarterRoom(supabase, ownerId, 'My room');
